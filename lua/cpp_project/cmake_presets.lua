...
-- configure preset into a CMake.Config (see cpp_project.cmake): fully
-- resolved (inherits chain + macro expansion + include files) so the menu
-- can display it and hand it straight to cmake.command_parts.
--
...
--- Resolve a configure preset into a CMake.Config, or nil if unknown.
--- `build_dir` is the fully expanded binaryDir (kept as-is, not reduced to a
--- basename - cmake.command_parts takes build_dir literally, so truncating
--- it here would build in the wrong directory whenever binaryDir isn't a
...
function M.resolve(root, name)
	...
	return {
		...
		build_dir = raw.binaryDir and expand(raw.binaryDir, ctx) or nil,
